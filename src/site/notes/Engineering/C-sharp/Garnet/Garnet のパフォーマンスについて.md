...
Get は Redis と比較しても大きな差はありませんが、MSET は高速なことがわかります。
...
io-threads を指定して Redis を起動し、再度 64 スレッドで試してみました。
...
### 余談: Garnet でスレッド数を増やしてみる
ローカルなのであまり意味がないかもしれませんが、512 スレッドまで試しました。
...
# 所感
MSET など一部の操作では Redis よりも高速なパフォーマンスを発揮することがわかりました。
とはいえ GET, SET のような基本的な操作では Garnet と Redis で大きな差はありませんでした。
但しそれは Redis が処理できる並列数の範囲内での話であり、スレッド数を増やすと急激にパフォーマンスが落ちる傾向があるように見え、Garnet はスレッド数を増やしても安定してパフォーマンスが出る傾向があるように思われます。
キャパシティプランニングにおいても Garnet の方が立てやすいかもしれません。
キャッシュ層を使い倒す、スケーリングをさせるような場合は Garnet を検討してみる価値はあるかもしれません。
また、PUB/SUB などは試していないので、興味があれば試してみると良いかもしれません。
